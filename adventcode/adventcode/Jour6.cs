...
{
	/*private static RangeLights getRangeLight(string range)
		{
			return (new RangeLights (Convert.ToInt32(range.Split (',') [0]), Convert.ToInt32(range.Split (',') [1])));
		}


		public static void turnon(ref LightStruct[,] LightStruct, RangeLights min, RangeLights max)
		{
			int start_x = min._x;
			int start_y = min._y;

			while (start_y <= max._y) {
			
				while (start_x <= max._x) {
					LightStruct [start_x, start_y]._isOn = true;
					start_x += 1;
				}
				start_y += 1;
			}
		...
		public static void turnoff(ref LightStruct[,] LightStruct, RangeLights min, RangeLights max)
		{
			int start_x = min._x;
			int start_y = min._y;
			...
			while (start_y <= max._y) {
				...
				while (start_x <= max._x) {
					LightStruct [start_x, start_y]._isOn = false;
					start_x += 1;
				}
				start_y += 1;
			}
		...
		public static void toggle(ref LightStruct[,] LightStruct, RangeLights min, RangeLights max)
		{
			int start_x = min._x;
			int start_y = min._y;
			while (start_y <= max._y) {
				while (start_x <= max._x) {
					LightStruct [start_x, start_y]._isOn = !LightStruct [start_x, start_y]._isOn;
					start_x += 1;
				}
				start_y += 1;
			}
	...
	public struct LightStruct
	{
		...
		public LightStruct(int x, int y, bool isOn)
		{
			_x = x;
			_y = y;
			_isOn = isOn;
	...
	public struct RangeLights
	{
		public int _x;
		public int _y;

		public RangeLights(int x, int y)
		{
			_x = x;
			_y = y;
		}
	...
	public class Jour6
	{
		//private delegate void DelLightsAction(ref LightStruct[,] lights, RangeLights rangeMin, RangeLights rangeMax);
		 
		//private static Dictionary<Command, DelLightsAction> doCommand = new Dictionary<Command, DelLightsAction>(3);
		...
		public static int GetResultPart1()
		{
			...
			//On transform grace a regex.match.groups
				.Select (i => Regex.Match (i, pattern).Groups)
				.Select (g => new {
					command = getCommand (g [1].Value),
					From = new {
				...
				.SelectMany (t => 
					from x in Enumerable.Range(t.From.X, 1 + t.To.X - t.From.X)
					from y in Enumerable.Range(t.From.Y, t.To.Y - t.From.Y + 1)
					select new {t.command, x, y });
				
						
			var lights = new bool[1000, 1000];
			foreach (var i in instructions)
			{    
				if (i.command == Command.TURNON) 
					lights[i.x, i.y] = true;
				else if (i.command == Command.TURNOFF)
					lights[i.x, i.y] = false;
				else if (i.command == Command.TOGGLE)
					lights[i.x, i.y] = !lights[i.x, i.y];
			}

			return (from x in Enumerable.Range(0, 1000)
		...
		public static int GetResultPart2()
		{
			...
			var results = input.Split ('\n')
				...
				.Select (g => new {
					...
					from x in Enumerable.Range(t.From.X,1 + t.To.X - t.From.X)
					from y in Enumerable.Range(t.From.Y,1 + t.To.Y - t.From.Y)
					select new {t.command, x, y}
				).Aggregate(new int[1000,1000], (acc, next) => {
					var brightness = acc[next.x,next.y];
					if (next.command == "turn on") 
						brightness+=1;
					else if (next.command == "turn off") 
						brightness = Math.Max(0, brightness - 1);
					else if (next.command == "toggle") 
						brightness += 2;
